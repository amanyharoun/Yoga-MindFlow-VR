...
namespace Core3lb
{
    public class GroKitXRGrabObject : BaseXRGrabObject
    {
        ...
        public void OnTriggerEnter(Collider other)
        {
            if (other.TryGetComponent(out XRHand curHand))
            {
                if(currentHand == null)
                {
                    ...
                    currentHandsInObject.AddIf(curHand);
                }
            }
        }
        ...
        public virtual void Update()
        {
            ...
            if (currentHand)
            {
                foreach (var otherHands in currentHandsInObject)
                {
            ...
            if (isGrabbed)
            {
                ...
                if(allowStealing)
                {
                    foreach (var otherHands in currentHandsInObject)
                    {
                        if(GetGrabInputDown(otherHands))
                        {
                            currentHand.currentHeldObject = null;
                            currentHand = otherHands;
                            Grab();
                            return;
                        }
                    }
