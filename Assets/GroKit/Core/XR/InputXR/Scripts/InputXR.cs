...
namespace Core3lb
{
    ...
    //PlayerRigBase.PlayerInput().GetInputBool(PlayerRigBase.PlayerInput().handTrigger, Input3lb.Controller.Left,Input3lb.InputRequest.Get);
        ...
        public void Awake()
            {
                if(instance != null)
                {
                    Debug.LogError("Two InputXR's detected deleting this one".ToDebugColor(), gameObject);
                    Destroy(this);
                }
                instance = this;
                if(inputOverride)
                {
                    ...
                    {
                        Debug.LogError("Slotted Override Input is not an IOverrideInput".ToDebugColor(), gameObject);
                    }
                }              
            }
        ...
        {
            if (overrideInputOverrideInput?.IenableOverride == true)
                return overrideInputOverrideInput.GetCustom1(which);
            return HandleReference(custom1, which, InputRequest.Get);
        }

        public virtual bool GetCustom2(Controller which)
        {
            if (overrideInputOverrideInput?.IenableOverride == true)
                return overrideInputOverrideInput.GetCustom2(which);
            return HandleReference(custom2, which, InputRequest.Get);
        }

        public virtual bool GetCustom3(Controller which)
        {
            if (overrideInputOverrideInput?.IenableOverride == true)
                return overrideInputOverrideInput.GetCustom3(which);
            return HandleReference(custom3, which, InputRequest.Get);
        }

        public virtual bool GetCustom4(Controller which)
        {
            if (overrideInputOverrideInput?.IenableOverride == true)
                return overrideInputOverrideInput.GetCustom4(which);
            return HandleReference(custom4, which, InputRequest.Get);
        }
        ...
        public virtual bool ButtonRequest(Button button, Controller which, InputRequest inputRequest)
            {
                switch (button)
                {
                    ...
                    case Button.Custom1:
                        return GetBool(custom1, which, inputRequest);
                    case Button.Custom2:
                        return GetBool(custom2, which, inputRequest);
                    case Button.Custom3:
                        return GetBool(custom3, which, inputRequest);
                    case Button.Custom4:
                        return GetBool(custom4, which, inputRequest);
                default:
                        return false;
                }
            }
        ...
        public virtual bool GetButton(Button button, Controller which,bool forceBase = false)
            {
                ...
                switch (button)
                {
                    ...
                    case Button.Custom1:
                        return GetCustom1(which);
                    case Button.Custom2:
                        return GetCustom2(which);
                    case Button.Custom3:
                        return GetCustom3(which);
                    case Button.Custom4:
                        return GetCustom4(which);
                default:
                        return false;
                }
            }
