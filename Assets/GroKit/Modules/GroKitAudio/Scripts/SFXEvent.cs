    ...
    public class SFXEvent : SFXBase
    {
        ...
        protected override void PlayerInternalComplex(AudioClip clip = null)
        {
            if(clip == null)
            {
                ...
                {
                    Debug.Log("SFXEVENT - No Clip Detected not Playing".ToDebugColor(StringExtensions.ColorType.Orange), gameObject);
                }
                return;
            }
            if (onlyPlayIfActive)
            {
                if (!gameObject.activeInHierarchy)
                {
                    if (GroKitAudioManager.showDebugs)
                    {
                        Debug.Log("SFXEVENT - Not Playing Because Off and onlyPlayIfActive is true".ToDebugColor(StringExtensions.ColorType.Orange), gameObject);
                    }
                    return;
                }
            }
