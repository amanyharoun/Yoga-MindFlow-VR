  ...
  //      /// <param name="pitch"></param>
		...
		public void DebugRun(AudioPooler myComponent, GameObject caller)
		{
			if (showDebugs)
			{
				...
				{
                    Debug.LogError($"AUDIO  Caller: {caller.name} |  Clip: {myComponent.mySource.clip.name} | Mixer: {myComponent.mySource.outputAudioMixerGroup.name} ".ToDebugColor(StringExtensions.ColorType.Orange), caller);
					if(myComponent.sourceTemplate)
					{
                        Debug.LogError($"AUDIO  Source Template: {caller.name} |  Clip: {myComponent.mySource.clip.name} | Mixer: {myComponent.mySource.outputAudioMixerGroup.name} | Template Name: {myComponent.sourceTemplate.name} ".ToDebugColor(StringExtensions.ColorType.Orange), myComponent.sourceTemplate);
                    }
                }
				else
				{
                    Debug.LogError($"AUDIO  Caller: {caller.name} |  Clip: {myComponent.mySource.clip.name} | Mixer: NO MIXER ".ToDebugColor(StringExtensions.ColorType.Orange), caller);
                    if (myComponent.sourceTemplate)
                    {
                        Debug.LogError($"AUDIO  Source Template: {caller.name} |  Clip: {myComponent.mySource.clip.name} | Mixer: NO MIXER | Template Name: {myComponent.sourceTemplate.name} ".ToDebugColor(StringExtensions.ColorType.Orange));
                    }
                }
